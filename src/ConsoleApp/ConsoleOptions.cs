...
namespace ConsoleApp
{
	...
	{
		[Option(Description="Allow Ld")]
		public bool ld;
		[Option(Description="Allow Hi")]
		public bool hi;
			...
			foreach (FieldInfo field in fields)
			{
				...
				if (atts.Length > 0)
				{
					...
					if (att.Description != null)
					{
						...
						helpText.AppendFormat("/{0,-20} ", field.Name+valType );

						const int maxRowLength = 50;
						string separator = new String(' ',22);
						string description = att.Description+(att.Short!=null?String.Format(" (Short format: /{0}{1})", att.Short, valType):String.Empty );

						int current = 0, last = 0;
						while( description.Length-current>maxRowLength ) {
							current = description.LastIndexOfAny( new char[] {' ','\t'}, Math.Min(description.Length,current+maxRowLength) );
							if( current<0 ) {
								current = last+maxRowLength;
							}
							helpText.Append( (last>0?separator:"")+description.Substring(last, current-last).Trim()+"\n" );
							last = current;
						}
						helpText.Append( (last>0?separator:"")+description.Substring(last, description.Length-last).Trim() );
